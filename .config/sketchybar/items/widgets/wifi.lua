...
wifi:subscribe({ "wifi_change", "system_woke" }, function(env)
	sbar.exec([[ipconfig getsummary en0 | awk -F ' SSID : '  '/ SSID : / {print $2}']], function(wifi_name)
		...
		sbar.exec("sleep 1 && /usr/local/bin/piactl get connectionstate", function(vpn)
			vpn = string.gsub(vpn, "%s+", "")
			local vpnconnected = (vpn == "Connected")

			if vpnconnected and connected then
